...
$downloadDirectory = if ($env:LMR_DOWNLOAD_DIR) { $env:LMR_DOWNLOAD_DIR } else { Join-Path ([Environment]::GetFolderPath("UserProfile")) "Downloads" }
if (-not (Test-Path -LiteralPath $downloadDirectory)) { New-Item -ItemType Directory -Path $downloadDirectory | Out-Null }
